...
public class ConvertToRelativePosition : MonoBehaviour {
    ...
    /// <summary>
    /// This is a utility script that allows you to easily convert Unity Canvas
    /// elements (anything with a RectTransform component) between absolute
    /// pixel units (where an offset value in pixels determines the size and
    /// positioning) and relative anchors (where anchor values between 0 and 1
    /// determine the size and positioning). This makes it much easier to
    /// lay out UI designs using Unity's tools, and then make those designs
    /// scale properly to screens of different resolutions.
    /// 
    /// Place this script in your project's Assets in a folder called Editor.
    /// You may already have this folder, or you may have to create it. The
    /// Editor folder is special because Unity checks it for scripts to run
    /// in Edit mode. It will create a new top level menu called Utilities
    /// with several menu options.
    /// </summary>
